...
export const selectPropertySetPrompt = async (message, multiSelect = false, // will trigger return of array of selected items
initiallySelectedPsetNames = []) => {
    ...
    if (pSets.length >= 1) {
        ...
        const prompt = new LocalSelect({
            ...
            choices: pSets.map((pSet) => ({
                ...
                hint: `has ${pSet.hasProperties.length} properties: ${pSet.hasProperties.map((p) => p.name.value).join(', ')}`,
            })),
            result(res) {
...
//# sourceMappingURL=select-property-set.js.map