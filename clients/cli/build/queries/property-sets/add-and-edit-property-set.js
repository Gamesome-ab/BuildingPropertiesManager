...
import { PropertySet, PropertySetRepository, Label, Text, } from '@building-properties-manager/core';
import { managePropertySetsPrompt } from './manage-property-set.js';
const { Input } = E;
export const handleAddPropertySet = async (currentlyEditing, currentPromptStep = 0) => {
    currentlyEditing = currentlyEditing ? currentlyEditing : new PropertySet(new Label('Pset_SomePsetName'), new Text('Some explicit description of what this will contain and how it will be used'));
    ...
    if (currentPromptStep === 1) {
        const description = await describePropertySetPrompt(currentlyEditing.name, currentlyEditing.description);
        if (!description) {
...
export const handleEditPropertySet = async (oldPropertySet, currentlyEditing, currentPromptStep = 0) => {
...
const describePropertySetPrompt = async (propertySetName, initial) => {
...
const namePropertySetPrompt = async (editablePropertySet, firstMessageLine) => {
    const propertySetRepository = new PropertySetRepository();
...
//# sourceMappingURL=add-and-edit-property-set.js.map